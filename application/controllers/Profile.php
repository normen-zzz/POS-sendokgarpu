...
class Profile extends CI_Controller
{
    ...
    private function _validasi()
    {
        ...
        $this->form_validation->set_rules('no_telp', 'Phone', 'required|trim|numeric');
    }
    ...
    public function setting()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (empty($_FILES['foto']['name'])) {
                ...
                if ($insert) {
                    set_pesan('profile changed successfully!');
                } else {
                    set_pesan('failed to change profile!');
                }
    ...
    {
        $this->form_validation->set_rules('password_lama', 'Old Password', 'required|trim');
        $this->form_validation->set_rules('password_baru', 'New Password', 'required|trim|min_length[3]|differs[password_lama]');
        $this->form_validation->set_rules('konfirmasi_password', 'Confirm Password', 'matches[password_baru]');
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (password_verify($input['password_lama'], userdata('password'))) {
                ...
                if ($query) {
                    ...
                    set_pesan('failed to change password!', false);
                }
            } else {
                set_pesan('old password is wrong!', false);
            }
